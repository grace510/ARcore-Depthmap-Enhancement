...
parser.add_argument("--preprocess", type=int, default=0, help='run prepare_data or not')
...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (imgn_train, img_train) in enumerate(loader_train, 0):
            ...
            out_train = model(imgn_train[1].unsqueeze(1))
            # loss = criterion(out_train, noise) / (imgn_train.size()[0]*2)
            loss = criterion(out_train, img_train[1].unsqueeze(1))
            loss.backward()
            ...
            out_train = torch.clamp(model(imgn_train[1].unsqueeze(1)), 0., 1.)
            psnr_train = batch_PSNR(out_train, img_train[1].unsqueeze(1), 1.)
            '''
            ...
            if i % int(len(loader_train)//5) == 0:
                ...
                for k in range(len(dataset_val)):
                    imgn_val = torch.unsqueeze(dataset_val[k][0], 0)
                    img_val = torch.unsqueeze(dataset_val[k][1], 0)
                    with torch.no_grad():
                        img_val, imgn_val = Variable(img_val.cuda()), Variable(imgn_val.cuda())
                        # out_val = torch.clamp(imgn_val-model(imgn_val), 0., 1.)
                        # print(imgn_val[1].size())
                        out_val = torch.clamp(model(imgn_val[:, 1, :, :].unsqueeze(1)), 0., 1.)
                    psnr_val += batch_PSNR(out_val, img_val[:, 1, :, :].unsqueeze(1), 1.)
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.mode == 'S':
            ...
            prepare_data(data_path='data', patch_size=96, stride=96, aug_times=1)
        if opt.mode == 'B':
